...
describe('API Pact test', () => {
  ...
  describe('retrieving a product', () => {
    ...
    // delete random test later
    test('ID 9 exists', async () => {
      // Arrange
      const expectedProduct = { id: '9', type: 'CREDIT_CARD', name: '28 Degrees'}
      ...
      await mockProvider.addInteraction({
        state: 'a product with ID 9 exists',
        uponReceiving: 'a request to get a product',
        withRequest: {
          method: 'GET',
          path: '/product/9',
      ...
      // Act
      const api = new API(mockProvider.mockService.baseUrl);
      const product = await api.getProduct('9');
